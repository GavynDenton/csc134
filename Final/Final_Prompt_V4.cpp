/*
Prompt: make the targets spawn on their own

Resposnse:
To make the targets spawn on their own without the need for user input, you can modify the code by introducing a timer-based approach. Here's an updated version of the code that includes automatic spawning of targets at regular intervals:
...
In this updated code, I added a `targetSpawnTimer` variable that keeps track of the time elapsed since the last target spawn. The `targetSpawnInterval` variable determines how frequently targets should be spawned (in this case, every 2 seconds).

Inside the game loop, the `targetSpawnTimer` is incremented by the frame time (`GetFrameTime()`) every frame. When the `targetSpawnTimer` exceeds the `targetSpawnInterval`, targets are spawned by iterating over the `targets` array and activating an inactive target.

Now, targets will automatically spawn at regular intervals without the need for user input.
